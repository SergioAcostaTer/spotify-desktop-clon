...
export const PlaylistHomeVert = ({ id = "" }) => {
  ...
  return (
    <div className="relative group w-full h-full">
        ...
        <div className="p-[16px] flex flex-col gap-y-[8px] bg-[#ffffff05] group-hover:bg-[#ffffff18] transition-all duration-300 rounded-[5px]">
          <div className="w-full h-full relative">
            <img
              ...
              alt={playlist?.name}
              className="rounded-[4px] w-full absolute bottom-0 left-0 z-[50]"
            />
            <div className="w-full aspect-square bg-gray-500 rounded-[4px] animate-pulse" />
          </div>
      ...
      <button
        className={`absolute w-[48px] h-[48px] rounded-full bg-green-500 bottom-[85px] right-[23px] opacity-0 group-hover:opacity-100 ${
          playing && "opacity-100"
